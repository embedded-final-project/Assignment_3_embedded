...
void main() {                              
 ...
 //uart_data(Temp_reading);                  //works if uart enabled (uncommented in macros)
 #endif
 ...
 while(1){
    ...
    Display_Temp(Temp_reading, user);       //function compares current temp reading with user input value and display status on lcd (decreasing/increasing)
    fan_operation(Temp_reading, user);      //function compares current temp reading with user input and accordingly operates (cooler/heater)
    //motor_on_uart();                      //works if uart enabled (uncommented in macros)
 }
 }
